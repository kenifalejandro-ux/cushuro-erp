...
const ITEM_VACIO: ItemForm = { etapa_actividad: '', peligro: '', riesgo: '', probabilidad: 1, severidad: 1, medidas_control: '' };
...
function EstadoBadge({ estado }: { estado: string }) {
  ...
  return <span className={`font-bold capitalize ${estilos[estado] ?? 'text-slate-500'}`}>{estado}</span>;
}
...
export default function IpercView() {
  ...
  const [formIperc, setFormIperc] = useState({
    ...
    area_frente: '', turno: '', equipo_id: '', tarea_especifica: '',
  });
  ...
  const actualizarItem = (items: ItemForm[], setItems: (v: ItemForm[]) => void, i: number, campo: keyof ItemForm, valor: string | number) => {
    const nuevos = [...items];
  ...
  const ItemsEditor = ({ items, setItems }: { items: ItemForm[]; setItems: (v: ItemForm[]) => void }) => (
      ...
      {items.map((item, i) => (
        <div key={i} className="bg-slate-50 rounded-xl p-4 space-y-2">
          <div className="grid grid-cols-2 gap-2">
            <input placeholder="Etapa/actividad" required className="border border-slate-200 rounded-lg p-2 text-sm outline-none"
              value={item.etapa_actividad} onChange={(e) => actualizarItem(items, setItems, i, 'etapa_actividad', e.target.value)} />
            <input placeholder="Peligro" required className="border border-slate-200 rounded-lg p-2 text-sm outline-none"
              value={item.peligro} onChange={(e) => actualizarItem(items, setItems, i, 'peligro', e.target.value)} />
          </div>
          <input placeholder="Riesgo" required className="w-full border border-slate-200 rounded-lg p-2 text-sm outline-none"
            value={item.riesgo} onChange={(e) => actualizarItem(items, setItems, i, 'riesgo', e.target.value)} />
          <div className="grid grid-cols-2 gap-2">
            <label className="text-xs text-slate-500 flex items-center gap-2">Probabilidad (1-4)
              <select className="border border-slate-200 rounded-lg p-1 text-sm" value={item.probabilidad}
                onChange={(e) => actualizarItem(items, setItems, i, 'probabilidad', Number(e.target.value))}>
                {[1, 2, 3, 4].map((n) => <option key={n} value={n}>{n}</option>)}
              </select>
            </label>
            <label className="text-xs text-slate-500 flex items-center gap-2">Severidad (1-4)
              <select className="border border-slate-200 rounded-lg p-1 text-sm" value={item.severidad}
                onChange={(e) => actualizarItem(items, setItems, i, 'severidad', Number(e.target.value))}>
                {[1, 2, 3, 4].map((n) => <option key={n} value={n}>{n}</option>)}
              </select>
            </label>
          </div>
          <textarea placeholder="Medidas de control" required className="w-full border border-slate-200 rounded-lg p-2 text-sm outline-none"
            value={item.medidas_control} onChange={(e) => actualizarItem(items, setItems, i, 'medidas_control', e.target.value)} />
          {items.length > 1 && (
            <button type="button" onClick={() => setItems(items.filter((_, idx) => idx !== i))} className="text-xs text-red-500 hover:underline">Quitar ítem</button>
          )}
        </div>
      ))}
      <button type="button" onClick={() => setItems([...items, { ...ITEM_VACIO }])} className="text-sm text-slate-600 hover:text-slate-900 font-medium">+ Agregar ítem</button>
    </div>
  );
    ...
    <div className="p-4 lg:p-8 animate-in fade-in duration-500">
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6 mb-8">
        <div>
          ...
          <p className="text-slate-500">Identificación de peligros, evaluación de riesgos y controles</p>
        </div>
        <div className="flex gap-2">
          <button onClick={() => setSubTab('iperc')} className={`px-4 py-2 rounded-xl font-medium text-sm ${subTab === 'iperc' ? 'bg-slate-900 text-white' : 'bg-white border border-slate-200 text-slate-600'}`}>IPERC</button>
          <button onClick={() => setSubTab('lineasBase')} className={`px-4 py-2 rounded-xl font-medium text-sm ${subTab === 'lineasBase' ? 'bg-slate-900 text-white' : 'bg-white border border-slate-200 text-slate-600'}`}>Líneas Base</button>
        </div>
      </div>
      ...
      {subTab === 'iperc' && (
        <>
          <div className="flex justify-end mb-6">
            <button onClick={() => setModalIpercAbierto(true)} className="px-6 py-2.5 bg-slate-900 hover:bg-slate-800 text-white font-medium rounded-xl transition-all">+ Nuevo IPERC</button>
          </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead className="bg-slate-50">
                <tr>
                  ...
                  <th className="p-5 text-xs font-bold text-slate-400 uppercase tracking-widest">área/frente</th>
                  ...
                  <th className="p-5 text-xs font-bold text-slate-400 uppercase tracking-widest text-right">acciones</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
                {ipercs.map((i) => (
                  <tr key={i.id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="p-5 font-mono text-sm text-slate-500">{i.placa_codigo || '---'}</td>
                    <td className="p-5 text-sm text-slate-500">{i.usuario_nombre}</td>
                    <td className="p-5 text-sm"><EstadoBadge estado={i.estado} /></td>
                    <td className="p-5 text-right space-x-1">
                      {i.estado === 'borrador' && (
                        <>
                          <button onClick={() => handleCambiarEstadoIperc(i.id, 'aprobado')} className="px-2 py-1 text-xs text-emerald-700 hover:bg-emerald-50 rounded-lg" title="Aprobar">✔ Aprobar</button>
                          <button onClick={() => handleCambiarEstadoIperc(i.id, 'rechazado')} className="px-2 py-1 text-xs text-red-600 hover:bg-red-50 rounded-lg" title="Rechazar">✕ Rechazar</button>
                        </>
                      )}
                      <button onClick={() => handleEliminarIperc(i.id)} className="p-2 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-all" title="Eliminar">🗑️</button>
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      {subTab === 'lineasBase' && (
        <>
          <div className="flex justify-end mb-6">
            <button onClick={() => setModalLineaBaseAbierto(true)} className="px-6 py-2.5 bg-slate-900 hover:bg-slate-800 text-white font-medium rounded-xl transition-all">+ Nueva Línea Base</button>
          </div>
          <div className="bg-white border border-slate-200 rounded-3xl overflow-hidden shadow-sm">
            <table className="w-full text-left border-collapse">
              <thead className="bg-slate-50">
                <tr>
                  <th className="p-5 text-xs font-bold text-slate-400 uppercase tracking-widest">proceso/actividad</th>
                  <th className="p-5 text-xs font-bold text-slate-400 uppercase tracking-widest">área/frente</th>
                  <th className="p-5 text-xs font-bold text-slate-400 uppercase tracking-widest">estado</th>
                  <th className="p-5 text-xs font-bold text-slate-400 uppercase tracking-widest text-right">acciones</th>
                </tr>
              ...
              <tbody className="divide-y divide-slate-100">
                {lineasBase.map((lb) => (
                  <tr key={lb.id} className="hover:bg-slate-50/50 transition-colors">
                    <td className="p-5 text-sm font-semibold text-slate-800">{lb.proceso_actividad}</td>
                    <td className="p-5 text-sm text-slate-500">{lb.area_frente || '---'}</td>
                    <td className="p-5 text-sm"><EstadoBadge estado={lb.estado} /></td>
                    <td className="p-5 text-right space-x-1">
                      {lb.estado === 'borrador' && (
                        <>
                          <button onClick={() => handleCambiarEstadoLineaBase(lb.id, 'aprobado')} className="px-2 py-1 text-xs text-emerald-700 hover:bg-emerald-50 rounded-lg">✔ Aprobar</button>
                          <button onClick={() => handleCambiarEstadoLineaBase(lb.id, 'rechazado')} className="px-2 py-1 text-xs text-red-600 hover:bg-red-50 rounded-lg">✕ Rechazar</button>
                        </>
                      )}
                      <button onClick={() => handleEliminarLineaBase(lb.id)} className="p-2 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-all" title="Eliminar">🗑️</button>
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex justify-center items-center z-50 p-4">
          <div className="bg-white w-full max-w-lg rounded-3xl shadow-2xl max-h-[90vh] overflow-y-auto">
            <div className="p-6 border-b flex justify-between items-center">
              ...
              <button onClick={() => setModalIpercAbierto(false)} className="text-slate-400 hover:text-slate-900 text-2xl">×</button>
            </div>
            <form onSubmit={handleCrearIperc} className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  <label className="text-xs font-bold text-slate-500 uppercase">Tipo</label>
                  <select className="w-full border border-slate-200 rounded-xl p-3 outline-none bg-white"
                    value={formIperc.tipo} onChange={(e) => setFormIperc({ ...formIperc, tipo: e.target.value as 'continuo' | 'especifico' })}>
                    <option value="continuo">Continuo</option>
                    <option value="especifico">Específico</option>
                  </select>
                </div>
                <div className="space-y-1">
                  <label className="text-xs font-bold text-slate-500 uppercase">Equipo (opcional)</label>
                  <select className="w-full border border-slate-200 rounded-xl p-3 outline-none bg-white"
                    value={formIperc.equipo_id} onChange={(e) => setFormIperc({ ...formIperc, equipo_id: e.target.value })}>
              ...
              <div className="space-y-1">
                ...
                <input type="text" required className="w-full border border-slate-200 rounded-xl p-3 outline-none"
                  value={formIperc.area_frente} onChange={(e) => setFormIperc({ ...formIperc, area_frente: e.target.value })} />
              </div>
              <div className="space-y-1">
                <label className="text-xs font-bold text-slate-500 uppercase">Turno (opcional)</label>
                <input type="text" className="w-full border border-slate-200 rounded-xl p-3 outline-none"
                  value={formIperc.turno} onChange={(e) => setFormIperc({ ...formIperc, turno: e.target.value })} />
              </div>
              {formIperc.tipo === 'especifico' && (
                <div className="space-y-1">
                  <label className="text-xs font-bold text-slate-500 uppercase">Tarea específica</label>
                  <input type="text" required className="w-full border border-slate-200 rounded-xl p-3 outline-none"
                    value={formIperc.tarea_especifica} onChange={(e) => setFormIperc({ ...formIperc, tarea_especifica: e.target.value })} />
                </div>
              )}
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex justify-center items-center z-50 p-4">
          <div className="bg-white w-full max-w-lg rounded-3xl shadow-2xl max-h-[90vh] overflow-y-auto">
            <div className="p-6 border-b flex justify-between items-center">
              ...
              <button onClick={() => setModalLineaBaseAbierto(false)} className="text-slate-400 hover:text-slate-900 text-2xl">×</button>
            </div>
            <form onSubmit={handleCrearLineaBase} className="p-6 space-y-4">
              <div className="space-y-1">
                <label className="text-xs font-bold text-slate-500 uppercase">Proceso/actividad</label>
                <input type="text" required className="w-full border border-slate-200 rounded-xl p-3 outline-none"
                  value={formLineaBase.proceso_actividad} onChange={(e) => setFormLineaBase({ ...formLineaBase, proceso_actividad: e.target.value })} />
              </div>
              <div className="space-y-1">
                <label className="text-xs font-bold text-slate-500 uppercase">Área/frente (opcional)</label>
                <input type="text" className="w-full border border-slate-200 rounded-xl p-3 outline-none"
                  value={formLineaBase.area_frente} onChange={(e) => setFormLineaBase({ ...formLineaBase, area_frente: e.target.value })} />
              </div>
