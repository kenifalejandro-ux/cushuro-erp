  ...
  // Si hay editingId es PUT (editar), si no es POST (crear)
    ...
    try {
      ...
      if (res.ok) {
        ...
        setEditingId(null);
        fetchRepuestos();
        setFormData({ codigo: '', nombre: '', categoria: 'General', stock: 0, stock_minimo: 5, stock_maximo: 30, precio: 0 });
      }
    } catch (err) {
  ...
  <table className="w-full text-left border-collapse">
    <thead className="bg-slate-50">
      <tr>
        ...
        <th className="p-5 text-xs font-bold text-slate-400 uppercase tracking-widest">codigo</th>
        <th className="p-5 text-xs font-bold text-slate-400 uppercase tracking-widest">nombre</th>
        <th className="p-5 text-xs font-bold text-slate-400 uppercase tracking-widest">categoria</th>
        <th className="p-5 text-xs font-bold text-slate-400 uppercase tracking-widest">stock</th>
        <th className="p-5 text-xs font-bold text-slate-400 uppercase tracking-widest">stock_minimo</th>
        <th className="p-5 text-xs font-bold text-slate-400 uppercase tracking-widest">stock_maximo</th>
        <th className="p-5 text-xs font-bold text-slate-400 uppercase tracking-widest">precio</th>
        <th className="p-5 text-xs font-bold text-slate-400 uppercase tracking-widest text-right">fecha-creación</th>
...
<td className="p-5 text-sm text-slate-400 font-medium">{r.stock_maximo}</td>
          ...
          <td className="p-5 text-sm font-medium text-slate-900">S/ {Number(r.precio).toFixed(2)}</td>
          
          {/* FECHA DE CREACIÓN (Mapeada desde el backend) */}
          <td className="p-5 text-sm text-slate-500 text-right">
            {r.fecha || '---'} 
          </td>
  ...
  <div className="space-y-1">
    <label className="text-xs font-bold text-slate-500 uppercase">Código del Repuesto</label>
    <input 
      type="text" placeholder="Ej: FIL-001" required
  ...
  <div className="grid grid-cols-2 gap-4">
    <div className="space-y-1">
      ...
      <input 
        type="number" className="w-full border border-slate-200 rounded-xl p-3 outline-none focus:ring-2 focus:ring-slate-900"
...
/>
  ...
  {/* Fila 5: Precio */}
  <div className="space-y-1">
    <label className="text-xs font-bold text-slate-500 uppercase">Precio Unitario (S/)</label>
    <input 
      type="number" step="0.01" className="w-full border border-slate-200 rounded-xl p-3 outline-none focus:ring-2 focus:ring-slate-900"
