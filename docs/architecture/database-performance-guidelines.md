...
**Importante al leer la salida del modo 2**: con pocas filas, un `Seq Scan` es el plan *correcto* — el planner de Postgres no usa un índice si escanear la tabla entera es más barato, y con pocas filas casi siempre lo es. Un `⚠` en este script solo es una señal real de problema cuando el tenant usado tiene volumen de datos comparable al de producción. `tests/db-index-coverage.test.ts` (índices faltantes) y este script (planes reales) son complementarios, no intercambiables — uno audita estructura, el otro comportamiento.
